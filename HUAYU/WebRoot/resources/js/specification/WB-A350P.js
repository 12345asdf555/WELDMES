...
//		mach = mach.toString(16);
	...
	+ fpulse_tuny_ele + fsarc_time + farc_tuny_ele + fsarc_init_time + 
		...
		socketfc.onmessage = function(msg) {
			...
			if (data.substring(0, 2) == "7E" && data.substring(10, 12) == "56") {
				if (data.substring(18, 20) == "FF") {
					...
					if(data.substring(90, 92).substring(0,2).toUpperCase()=="FF"){
						$("#farc_tuny_ele").numberbox('setValue', parseInt("FFFF"+data.substring(90, 92), 16)<<0);
					}else{
						$("#farc_tuny_ele").numberbox('setValue', parseInt(data.substring(90, 92), 16));
					}
					...
					if (parseInt(data.substring(76, 78), 16) == "01") {
						$('#ac_form').combobox('select', "01");
					} else if (parseInt(data.substring(76, 78), 16) == "02") {
						$('#ac_form').combobox('select', "02");
					} else {
						$('#ac_form').combobox('select', "03");
					}
					if (parseInt(data.substring(78,80), 16) == 1) {
						$('#weld_method').combobox('select', "1");
					} else if (parseInt(data.substring(78,80), 16) == 2) {
						$('#weld_method').combobox('select', "2");
					} else if (parseInt(data.substring(78,80), 16) == 3) {
					...
					if (parseInt(data.substring(88, 90), 16) == 1) {
...
function WBAPINIT(value) {
	...
	$('#fsarc_time').combobox('select', 1);//特殊收弧时序
	...
	$('#frepeat_end').combobox('select', 1);//反复时的结束方法
	...
	$('#ac_form').combobox('select',"01");//ac波形
}
...
function WBAPRULE() {
	...
	$("#weld_method").combobox({
		onSelect : function(record) {
			...
			if(record.value != "2" && $('#pulse_ctrl').is(':checked')) {
	...
	$("#frepeat_end").combobox({
		onSelect : function(record) {
			...
			if (record.value == "0" || $('#fsarc').is(':checked')) {
