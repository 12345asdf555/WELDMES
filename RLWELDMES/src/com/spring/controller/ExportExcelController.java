...
public class ExportExcelController {
	...
	public ResponseEntity<byte[]> exporItemdata(HttpServletRequest request,HttpServletResponse response){
		...
		try{
			...
			String[] titles = new String[]{"所属班组","设备总数","开机设备数","实焊设备数","设备利用率(%)","焊接焊缝数","焊接时间","工作时间","焊接效率(%)","焊丝消耗(G)","电能消耗(KWH)","气体消耗(L)","碳弧气刨时间"};
			...
			for(DataStatistics i:list){
				...
				if(arcairtime == null || "".equals(arcairtime)){
					...
					data[ii][12]=getTimeStrBySecond(arcairtime);
				}
				if(junction.getJunctionnum()!=0){
					...
					data[ii][10]=String.format("%.1f",electric*1.25);//电能消耗
				}else{
					data[ii][2]=0;
					data[ii][5]=0;
					data[ii][7]="00:00:00";
...
//							wire = (double)Math.round(wireweight*(Integer.valueOf(String.valueOf(weldtime.getWorktime()))*(parameter.getSpeed()/60))*100000)/100; //焊丝消耗量=焊丝长度*焊丝密度
	...
	public ResponseEntity<byte[]> exportItemWelddata(HttpServletRequest request,HttpServletResponse response){
		...
		try{
			...
			String[] titles = new String[]{"所属班组","累计焊接时间","正常段时长","超规范时长","规范符合率(%)","碳弧气刨时间"};
			...
			for(DataStatistics i:ilist){
				if(ii<ilist.size()){
					...
					BigInteger arcairtime = null;
					arcairtime = dss.getArcairtime4(dto);//获取碳弧气刨时间
					if(arcairtime == null || "".equals(arcairtime)){
					data[ii][5]="00:00:00";
					}else{
						data[ii][5]=getTimeStrBySecond(arcairtime);
					}
	...
	public ResponseEntity<byte[]> exportMachineData(HttpServletRequest request,HttpServletResponse response){
		...
		try{
			...
			String[] titles = new String []{"所属班组","设备编号","焊接焊缝数","焊接时间","工作时间","焊接效率(%)","焊丝消耗(G)","电能消耗(KWH)","气体消耗(L)","碳弧气刨时间"};
			...
			for(DataStatistics i:list){
				if(ii<list.size()){
					...
					if(junctionnum.getJunctionnum()!=0){
						...
						data[ii][7]=String.format("%.1f",electric*1.25);//电能消耗
					}else{
						data[ii][2]=0;
						data[ii][4]="00:00:00";
						data[ii][7]=0;
					...
					if(weld!=null){
						...
						if(arcairtime == null || "".equals(arcairtime)){
							...
							data[ii][9]=getTimeStrBySecond(arcairtime);
						}
...
//								wire = (double)Math.round(wireweight*(Integer.valueOf(String.valueOf(weld.getWorktime()))*(parameter.getSpeed()/60))*100000)/100; //焊丝消耗量=焊丝长度*焊丝密度
	...
	public ResponseEntity<byte[]> exportMachineWelddata(HttpServletRequest request,HttpServletResponse response){
		...
		try{
			...
			String[] titles = new String[]{"设备编码","所属班组","累计焊接时间","正常段时长","超规范时长","规范符合率(%)","碳弧气刨时间"};
			Object[][] data = new Object[ilist.size()][7];
			int ii=0;
			for(DataStatistics i:ilist){
				if(ii<ilist.size()){
					...
					BigInteger arcairtime = null;
					arcairtime = dss.getArcairtime5(dto);//获取碳弧气刨时间
					if(arcairtime == null || "".equals(arcairtime)){
						data[ii][6]="00:00:00";
					}else{
						data[ii][6]=getTimeStrBySecond(arcairtime);
					}
				}
				ii++;
			}
	...
	public ResponseEntity<byte[]> exportPersonData(HttpServletRequest request,HttpServletResponse response){
		...
		try{
			...
			String[] titles = new String []{"焊工编号","焊工名称","焊接焊缝数","焊接时间","工作时间","焊接效率(%)","焊丝消耗(G)","电能消耗(KWH)","气体消耗(L)","碳弧气刨时间"};
			Object[][] data = new Object[list.size()][10];
			int ii=0;
			for(DataStatistics i:list){
				if(ii<list.size()){
...
//								wire = (double)Math.round(wireweight*(Integer.valueOf(String.valueOf(weld.getWorktime()))*(parameter.getSpeed()/60))*100000)/100; //焊丝消耗量=焊丝长度*焊丝密度
	...
	public ResponseEntity<byte[]> exportPersonWelddata(HttpServletRequest request,HttpServletResponse response){
		...
		try{
			...
			String[] titles = new String[]{"焊工编号","焊工姓名","累计焊接时间","正常段时长","超规范时长","规范符合率(%)","碳弧气刨时间"};
			Object[][] data = new Object[ilist.size()][7];
			int ii=0;
			for(DataStatistics i:ilist){
				if(ii<ilist.size()){
	...
	public ResponseEntity<byte[]> exportWorkpieceData(HttpServletRequest request,HttpServletResponse response){
		...
		try{
			...
			String[] titles = new String []{"焊缝编号","焊接时间","工作时间","焊接效率(%)","焊丝消耗(G)","电能消耗(KWH)","气体消耗(L)","行走速度(cm/s)","热输入量(J/cm)","碳弧气刨时间"};
			Object[][] data = new Object[list.size()][10];
			int ii=0;
			for(DataStatistics i:list){
				if(ii<list.size()){
					...
					if(worktime!=null){
						...
						data[ii][5]=String.format("%.1f",electric*1.25);//电能消耗
...
//								wire = (double)Math.round(wireweight*(Integer.valueOf(String.valueOf(weld.getWorktime()))*(parameter.getSpeed()/60))*100000)/100; //焊丝消耗量=焊丝长度*焊丝密度
	...
	public ResponseEntity<byte[]> exportWorkpieceWelddata(HttpServletRequest request,HttpServletResponse response){
		...
		try{
			...
			String[] titles = new String[]{"焊缝编号","累计焊接时间","正常段时长","超规范时长","规范符合率(%)","碳弧气刨时间"};
			Object[][] data = new Object[ilist.size()][6];
			int ii=0;
			for(DataStatistics i:ilist){
				if(ii<ilist.size()){
					...
					BigInteger arcairtime = null;
					arcairtime = dss.getArcairtime7(dto);//获取碳弧气刨时间
					if(arcairtime == null || "".equals(arcairtime)){
						data[ii][5]="00:00:00";
					}else{
						data[ii][5]=getTimeStrBySecond(arcairtime);
