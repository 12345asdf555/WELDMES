var flag;
var needChanelCheck="",needGatherCheck="";
var client,clientId;
$(function() {
	QinTronRULE();
	QinTronMaterial();
...
function QinTron(yshu,gather){
	if(yshu==null){
		...
		if(fselect == 102){
			...
			if($('#farc_tuny_vol').numberbox('getValue')<0){
				farc_vol = parseInt($('#farc_tuny_vol').numberbox('getValue')>>>0).toString(16).toLocaleUpperCase().substring(4);
			}else{
				farc_vol = parseInt($('#farc_tuny_vol').numberbox('getValue')).toString(16);
			}
			if($('#fini_tuny_vol').numberbox('getValue')<0){
				fini_vol = parseInt($('#fini_tuny_vol').numberbox('getValue')>>>0).toString(16).toLocaleUpperCase().substring(4);
			}else{
				fini_vol = parseInt($('#fini_tuny_vol').numberbox('getValue')).toString(16);
			}
			if($('#fweld_tuny_vol').numberbox('getValue')<0){
				fweld_vol = parseInt($('#fweld_tuny_vol').numberbox('getValue')>>>0).toString(16).toLocaleUpperCase().substring(4);
			}else{
				fweld_vol = parseInt($('#fweld_tuny_vol').numberbox('getValue')).toString(16);
			}
		}
		...
		if(fselect == 102){
			...
			if(yshu.farc_tuny_vol<0){
				farc_vol = parseInt(yshu.farc_tuny_vol>>>0).toString(16).toLocaleUpperCase().substring(4);
			}else{
				farc_vol = parseInt(yshu.farc_tuny_vol).toString(16);
			}
			if(yshu.fini_tuny_vol<0){
				fini_vol = parseInt(yshu.fini_tuny_vol>>>0).toString(16).toLocaleUpperCase().substring(4);
			}else{
				fini_vol = parseInt(yshu.fini_tuny_vol).toString(16);
			}
			if(yshu.fweld_tuny_vol<0){
				fweld_vol = parseInt(yshu.fweld_tuny_vol>>>0).toString(16).toLocaleUpperCase().substring(4);
			}else{
				fweld_vol = parseInt(yshu.fweld_tuny_vol).toString(16);
			}
		}
	...
	needGatherCheck=mach;
	needChanelCheck=fchanel
	...
	;
	xxx = "7E" + data_length + "01020152" + xiafasend1;
	var check = 0;
	...
	client.onMessageArrived = function(e){
		...
		if (fan.substring(0, 2) == "7E" && fan.substring(10, 12) == "52") {
			if (needChanelCheck.toUpperCase() == fan.substring(16, 18) && needGatherCheck.toUpperCase() == fan.substring(12, 16)) {
				client.unsubscribe("webdataup", {
					onSuccess : function(e) {
						console.log("取消订阅成功");
					},
				...
				$('#buttonCancel').linkbutton('enable');
				$('#buttonOk').linkbutton('enable');
				if (parseInt(fan.substring(18, 20), 16) != 0 && parseInt(fan.substring(18, 20), 16) != 21) {
					alert("下发失败,错误代码:" + (parseInt(fan.substring(18, 20), 16)).toString());
				} else if (parseInt(fan.substring(18, 20), 16) == 21) {
					alert("焊机正在焊接，请稍后下发！");
				} else {
					QinTronSAVE(1);
					alert("下发成功");
...
//索取规范并赋值
function QinTronGET(alertFlag) {
	if(alertFlag!=-1){
		$('#buttonCancel').linkbutton('disable');
		$('#buttonOk').linkbutton('disable');
	}
...
function QinTronINIT(value) {
	if(value==1){
		...
		$("#fini_tuny_vol").numberbox('setValue', 0);
...
function QinTronSAVE(value) {
	...
	if(machineModel == 184){
		...
		+ "&frequency=" + frequency + "&ftime=" + ftime + "&machine=" + machine + "&farc_tuny_vol=" + farc_tuny_vol + "&fini_tuny_vol=" + fini_tuny_vol + "&machine=" + machine;
	}
	$.ajax({
		type : "post",
		async : false,
