...
//				console.log(redata);
				...
				time.push(Date.parse(ttme.substring(0, ttme.length-2)));
			    $("#r8").html(parseFloat((parseInt(redata.substring(95 + i, 99 + i), 10)/10).toFixed(1)) + " m/min");
			    $("#r9").html(parseFloat((parseInt(redata.substring(103 + i, 107 + i), 10)/10).toFixed(1)) + " L/min");
...
//				$("#r6").html(parseInt(redata.substring(91 + i, 95 + i), 10));
				...
				if (time.length != 0 && z < time.length) {
					var arcair  = parseInt(redata.substring(99 + i, 103 + i),10);
					if(arcair == 12){
						$("#arcair").val("已启用");
						$("#arcair").css("background-color", "#55a7f3");
					}
					else{
						$("#arcair").val("未开启");
						$("#arcair").css("background-color", "#818181");
					}
					...
					case "21":
						...
						break;
					case "22":
						$("#l5").val("开机自检保护（IGBT部分）");
						$("#l5").css("background-color", "#fe0002");
						$("#mrjpg").attr("src", "resources/images/welder_"+imgnum+"2.png");
						break;
					case "24":
						$("#l5").val("电流采样值与设定值偏差较大");
						$("#l5").css("background-color", "#fe0002");
						$("#mrjpg").attr("src", "resources/images/welder_"+imgnum+"2.png");
						break;
					case "26":
